...
describe('consent governance and user routes', () => {
  ...
  it('returns consent snapshots with audit trail metadata and outstanding required counts', async () => {
    const user = await User.create({
      firstName: 'Morgan',
      lastName: 'Lee',
      email: 'morgan.lee@gigvora.test',
      password: '$2b$10$hashedpasswordvalue',
      userType: 'user',
    });

    const marketingPolicy = await createConsentPolicy(
      {
        title: 'Marketing Preferences',
        code: 'marketing_preferences_snapshot',
        audience: 'user',
        region: 'global',
        legalBasis: 'consent',
        required: false,
      ...
      {
        summary: 'Optional marketing updates and curated events.',
        effectiveAt: '2024-04-10T00:00:00Z',
      },
      { actorId: 'ops-admin-98' },
    );

    const contractPolicy = await createConsentPolicy(
      {
        title: 'Core Platform Terms',
        code: 'core_terms_snapshot',
        audience: 'user',
        region: 'global',
        legalBasis: 'contract',
        required: true,
      ...
      {
        summary: 'Required terms for using Gigvora services.',
        effectiveAt: '2024-01-01T00:00:00Z',
      },
      { actorId: 'legal-admin-32' },
    );

    await request(app)
      .put(`/api/users/${user.id}/consents/${marketingPolicy.code}`)
      .set('Authorization', `Bearer ${buildUserToken(user.id)}`)
      .send({
        status: 'granted',
    ...
    expect(response.body.outstandingRequired).toBe(1);
    expect(response.body.policies).toHaveLength(2);

    const marketingEntry = response.body.policies.find((entry) => entry.policy.code === marketingPolicy.code);
    expect(marketingEntry.consent.status).toBe('withdrawn');
    expect(Array.isArray(marketingEntry.auditTrail)).toBe(true);
    expect(marketingEntry.auditTrail.length).toBeGreaterThanOrEqual(3);
    const auditActions = marketingEntry.auditTrail.map((event) => event.action);
    expect(auditActions).toEqual(
      expect.arrayContaining(['policy_created', 'policy_version_activated', 'consent_withdrawn']),
    );

    const contractEntry = response.body.policies.find((entry) => entry.policy.code === contractPolicy.code);
    expect(contractEntry.consent).toBeNull();
    expect(contractEntry.auditTrail.map((event) => event.action)).toEqual(
      expect.arrayContaining(['policy_version_activated', 'policy_created']),
    );
    expect(contractEntry.auditTrail.every((event) => event.actor && event.occurredAt)).toBe(true);
  });
