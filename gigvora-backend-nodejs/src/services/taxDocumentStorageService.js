...
const DEFAULT_STORAGE_ROOT = path.join(process.cwd(), 'storage', 'tax-documents');
...
export async function storeTaxDocument({ data, fileName, contentType, actorId }, { storageRoot } = {}) {
  ...
  if (buffer.length > MAX_FILE_SIZE_BYTES) {
    throw new ValidationError('Uploaded document exceeds the 20MB size limit.');
  }
  ...
  const storageKey = `tax/${relativePath}`;
...
export async function readTaxDocument(key, { storageRoot } = {}) {
  const resolvedStorageRoot = storageRoot ? path.resolve(storageRoot) : DEFAULT_STORAGE_ROOT;
  const relative = ensureString(key, 'key').replace(/^tax\//, '');
