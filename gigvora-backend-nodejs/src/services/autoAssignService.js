...
export async function buildAssignmentQueue({
  ...
  return sequelize.transaction(async (transaction) => {
    ...
    scored.sort((a, b) => b.score - a.score || a.priorityBucket - b.priorityBucket);
    ...
    await Promise.all(
      selected.map(async ({ freelancer, metrics }) => {
    ...
    return createdEntries.map((entry, index) => ({
      ...
      position: index + 1,
      freelancer: sanitizeUser(selected[index].freelancer),
      breakdown: selected[index].breakdown,
