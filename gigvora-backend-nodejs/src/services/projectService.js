...
export async function createProject(payload = {}, { actorId } = {}) {
  ...
  const numericActor = Number(actorId ?? payload.actorId ?? payload.ownerId);
  const ownerId = Number.isFinite(numericActor) && numericActor > 0 ? numericActor : null;
  if (!ownerId) {
    throw new ValidationError('An authenticated actorId is required to create a project.');
  }
