...
2. **Functionality.** Desktop shells bind role switcher, persona dashboards, inbox preview, notification bell, creation studio CTA, marketing mega menus, and omni-search together with guarded callbacks. Keyboard shortcuts, analytics, and connection-state chips make every path—authenticated and marketing—actionable. Marketing mega menus now hydrate from the `/site/navigation` tree service, with the header coordinating `fetchSiteNavigation`, `publicSite.js`, and `navigationTransformers.js` so backend metadata and icons light up live panels and search. 【F:gigvora-frontend-reactjs/src/components/Header.jsx†L8-L42】【F:gigvora-frontend-reactjs/src/services/publicSite.js†L58-L65】【F:gigvora-frontend-reactjs/src/utils/navigationTransformers.js†L1-L82】
...
5. **Placeholders Or non-working functions or stubs.** Live inbox hydration, logout wiring, analytics callbacks, and marketing journeys replace placeholder CTAs and lorem ensuring no inert shell interactions remain. The navigation schema upgrade and marketing seed align display types, metadata, and content so production shells never fall back to placeholder menus. 【F:gigvora-backend-nodejs/database/migrations/20241101120000-upgrade-site-navigation-structure.cjs†L1-L43】【F:gigvora-backend-nodejs/database/seeders/20241101121500-site-navigation-marketing-seed.cjs†L1-L206】【F:gigvora-backend-nodejs/src/services/siteManagementService.js†L1295-L1462】
...
2. **Functionality.** Drawer orchestrates role switching, primary nav, marketing exploration, search, quick links, language selection, analytics, and logout with transitions tuned for touch. The same backend-fed navigation tree powers `MobileMegaMenu` and search so marketing exploration stays current on small screens. 【F:gigvora-frontend-reactjs/src/components/Header.jsx†L8-L42】【F:gigvora-frontend-reactjs/src/components/navigation/MobileNavigation.jsx†L1-L120】【F:gigvora-frontend-reactjs/src/components/navigation/MobileMegaMenu.jsx†L1-L120】
