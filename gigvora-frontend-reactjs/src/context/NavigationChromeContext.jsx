...
const fallbackChrome = Object.freeze({
  ...
  footer: {
    ...
    officeLocations: ['London', 'Toronto', 'Lisbon', 'Singapore'],
    certifications: ['SOC 2 Type II', 'ISO 27001', 'GDPR ready', 'UK GDPR'],
    socialLinks: [
...
export function NavigationChromeProvider({ children }) {
  ...
  const refresh = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      console.error('Failed to load navigation chrome metadata', error);
      setState((previous) => ({
        ...previous,
        loading: false,
        error: error?.message ?? 'Unable to load navigation chrome metadata.',
...
NavigationChromeProvider.propTypes = {
...
export function useNavigationChrome() {
  ...
  if (!context) {
    throw new Error('useNavigationChrome must be used within a NavigationChromeProvider');
  }
  return context;
}

export default NavigationChromeContext;
