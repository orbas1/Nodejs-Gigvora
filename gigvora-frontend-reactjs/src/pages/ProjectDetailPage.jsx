...
export default function ProjectDetailPage() {
  ...
  const canManage = status === 'authenticated' && Boolean(user?.id);
  ...
  const handleSave = async (event) => {
    ...
    if (!formState) return;
    if (!canManage) {
      setError(new Error('You need to sign in to update project settings.'));
      return;
    }
    setSaving(true);
    setError(null);
  ...
  const handleRegenerate = async () => {
    ...
    if (!canManage) {
      setError(new Error('You need to sign in to refresh the queue.'));
      return;
    }
    setSaving(true);
    setError(null);
      ...
      <div className="relative mx-auto max-w-6xl space-y-10 px-6">
        ...
        {!canManage ? (
          <div className="rounded-3xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
            Sign in to edit project metadata, regenerate auto-assign queues, and record assignment events.
          </div>
        ) : null}
