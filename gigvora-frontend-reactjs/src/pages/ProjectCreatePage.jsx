...
export default function ProjectCreatePage() {
  ...
  const canCreate = status === 'authenticated' && Boolean(user?.id);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!canCreate) {
      setError('You need to be signed in to launch a project and trigger auto-assign.');
      return;
    }
    setSubmitting(true);
    setError(null);
      ...
      <div className="relative mx-auto max-w-4xl space-y-10 px-6">
        ...
        {!canCreate ? (
          <div className="rounded-3xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm text-amber-700">
            Sign in to capture your project details, enable fairness-weighted auto-assign, and sync milestones with escrow.
          </div>
        ) : null}
        <form
