...
import FeedCard, { FEED_CARD_DEFAULT_EDIT_DRAFT } from '../components/feed/FeedCard.jsx';
...
import {
  ...
  resolvePostType,
} from '../components/feed/feedNormalizers.js';
...
const TIMEFRAME_LIMITS = Object.freeze({
  '24h': 24 * 60 * 60 * 1000,
  '7d': 7 * 24 * 60 * 60 * 1000,
...
function getPostTimestamp(post) {
  const candidate = post?.createdAt ?? post?.publishedAt ?? post?.updatedAt;
  if (!candidate) {
    return null;
  }
...
function matchesCategory(post, category) {
  ...
  if (category === 'opportunities') {
    return OPPORTUNITY_POST_TYPES.has(type);
  }
  if (category === 'media') {
    return type === 'media' || (Array.isArray(post.mediaAttachments) && post.mediaAttachments.length > 0);
  }
  if (category === 'milestones') {
    return (
      type === 'project' ||
      type === 'gig' ||
      (Array.isArray(post.tags) && post.tags.some((tag) => /milestone|launch|retro/i.test(tag)))
    );
  }
  if (category === 'community') {
    return (
      type === 'news' ||
      type === 'launchpad' ||
      (Array.isArray(post.categories) && post.categories.some((cat) => /community|culture|team/i.test(cat)))
    );
  }
  return true;
}

function matchesAudience(post, audience) {
  if (!audience || audience === 'all') {
    return true;
  }
  const scope = (post?.visibility || post?.audience || post?.scope || '').toString().toLowerCase();
  if (audience === 'network') {
    return scope === 'network' || Boolean(post?.isNetworkOnly || post?.viewerIsFollower || post?.viewerIsConnection);
  }
  if (audience === 'org') {
    return Boolean(post?.orgId || post?.companyId || post?.workspaceId || post?.organisationId || post?.isOrgScoped);
  }
  if (audience === 'saved') {
    return Boolean(post?.viewerHasSaved || post?.savedAt || post?.viewerBookmarkId || post?.isPinned);
  }
  return true;
}

function matchesTimeframe(post, timeframe, now = Date.now()) {
...
export default function FeedPage() {
  ...
  const [editingDraft, setEditingDraft] = useState(FEED_CARD_DEFAULT_EDIT_DRAFT);
  ...
  const filteredPosts = useMemo(() => {
    ...
    return allPosts.filter(
      (post) => matchesCategory(post, filters.category) && matchesTimeframe(post, filters.timeframe, now) && matchesAudience(post, filters.audience),
    );
  }, [allPosts, filters]);

  const hasActiveFilters = filters.category !== 'all' || filters.timeframe !== '30d' || filters.audience !== 'all';
  ...
  const activityMetrics = useMemo(() => {
    ...
    filteredPosts.forEach((post) => {
      if (matchesCategory(post, 'opportunities')) {
        opportunityCount += 1;
      }
      if (matchesCategory(post, 'media')) {
        mediaCount += 1;
      }
      engagementTotal += countTotalEngagement(post);
      collectTopics(post).forEach((topic) => {
    ...
    const trendingTopics = sortedTopics.map(([topic, count]) => ({
      id: topic,
      label: topic.replace(/\b\w/g, (char) => char.toUpperCase()),
      count,
      percentage: count / maxTopicCount,
    }));

    const averageEngagement = filteredPosts.length ? Math.round((engagementTotal / filteredPosts.length) * 10) / 10 : 0;

    return {
      totalPosts: filteredPosts.length,
      opportunityPosts: opportunityCount,
  ...
  const lastRefreshedAt = data?.generatedAt ?? data?.updatedAt ?? lastUpdated ?? null;
  ...
  }, [filteredPosts, loading, fromCache]);
  ...
  const handleEditCancel = useCallback(() => {
    ...
    setEditingDraft(FEED_CARD_DEFAULT_EDIT_DRAFT);
    setEditingError(null);
  }, []);
  ...
  const renderFeedPost = useCallback(
    (post) => (
      <FeedCard
        ...
        editDraft={editingPostId === post.id ? editingDraft : FEED_CARD_DEFAULT_EDIT_DRAFT}
      ...
      return (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500">
          {loading
            ? 'Syncing timeline…'
            : allPosts.length
            ? 'No updates match your filters. Adjust filters to explore more activity.'
          ...
          <div className="grid gap-6 lg:grid-cols-12 lg:items-start">
            ...
            <div className="order-1 space-y-6 lg:order-2 lg:col-span-6">
              ...
              <ActivityFilters
                ...
                disabled={loading && !allPosts.length}
