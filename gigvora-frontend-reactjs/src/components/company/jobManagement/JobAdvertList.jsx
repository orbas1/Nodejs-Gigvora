        ...
        <div className="space-y-3">
          {(jobs ?? []).length ? (
            jobs.map((job, index) => {
              ...
              const cardKey = jobId ?? `job-${index}`;
              const isSelected =
                selectedJobId != null &&
                (selectedJobId === jobId || selectedJobId === job.job?.id);
              ...
              const bestMatch =
                Array.isArray(job.keywordMatches) && job.keywordMatches.length
                  ? job.keywordMatches[0]
                  : null;
              const matchScore = bestMatch ? Number(bestMatch.score) : null;
              const matchPercentage =
                bestMatch && Number.isFinite(matchScore)
                  ? `${Math.round(matchScore * 100)}%`
                  : null;
              const jobLink = jobId ? `/jobs/${encodeURIComponent(jobId)}` : null;

              const handleActivate = () => onSelect?.(job);
              ...
              return (
                <article
                  key={cardKey}
                  {...interactiveProps}
                  className={`w-full rounded-2xl border px-4 py-4 text-left shadow-sm transition focus:outline-none focus:ring-2 focus:ring-blue-300 ${
                    isSelected
                ...
                >
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-500">
                    ...
                    <span aria-hidden="true">•</span>
                    <span>{(advert.remoteType ?? 'remote').replace(/_/g, ' ')}</span>
                    {bestMatch ? (
                      <>
                        <span aria-hidden="true">•</span>
                        <span>
                          {bestMatch.candidateName ?? 'Match'}
