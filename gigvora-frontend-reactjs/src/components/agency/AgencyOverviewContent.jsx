...
const OVERVIEW_METRICS = [
  { id: 'clients', label: 'Active clients', value: 18, hint: '4 onboarding' },
  { id: 'projects', label: 'Managed projects', value: 42, hint: '8 in kickoff' },
  { id: 'talent', label: 'Bench capacity', value: '63%', hint: '120 hours available' },
];
...
const TEAM_TASKS = [
  {
    ...
    description: 'Review feedback and confirm follow-ups with account owners.',
  },
  {
    id: 'payments',
    title: 'Finance review',
    description: 'Approve payouts and reconcile open vendor invoices.',
  },
  {
    id: 'growth',
    title: 'Growth pipeline',
    description: 'Align on demos and proposals scheduled for next week.',
  },
];
...
export default function AgencyOverviewContent({ displayName }) {
  return (
    <div className="space-y-10">
      <section className="rounded-3xl border border-slate-200 bg-white p-8 shadow-soft">
        <header className="flex flex-col gap-2 border-b border-slate-200 pb-6">
          <p className="text-xs uppercase tracking-[0.4em] text-slate-500">Agency control tower</p>
          <h1 className="text-3xl font-semibold text-slate-900">Hello, {displayName}</h1>
          <p className="max-w-3xl text-sm text-slate-600">
            Track delivery health, client momentum, and bench availability at a glance. Use the actions below to keep teams and
            freelancers aligned.
          </p>
        </header>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-3">
          {OVERVIEW_METRICS.map((metric) => (
            <div
              key={metric.id}
              className="rounded-3xl border border-slate-200 bg-slate-50/80 p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-accent/60"
            >
