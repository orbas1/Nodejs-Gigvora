...
export default function AgencyOverviewSection({ displayName }) {
  return (
    <section id="agency-overview" className="space-y-12">
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-8">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-slate-900">Hello, {displayName ?? 'Agency team'}</h1>
      ...
      <section id="agency-focus" className="grid gap-8 lg:grid-cols-[1.35fr_1fr]">
            ...
            <div className="mt-4 space-y-3">
              {BENCH_SIGNALS.map((signal) => (
                ...
                >
                  ...
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold ${signal.badgeClassName}`}>
                    {signal.status}
                  </span>
                </div>
              ))}
        ...
        <aside id="agency-finance" className="space-y-6">
