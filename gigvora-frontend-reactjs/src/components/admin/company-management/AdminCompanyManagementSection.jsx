...
import CompanyEditorModal from './CompanyEditorModal.jsx';
import {
  listAdminCompanies,
  createAdminCompany,
  updateAdminCompany,
  archiveAdminCompany,
} from '../../../services/adminCompanyManagement.js';
...
export default function AdminCompanyManagementSection() {
  ...
  const openEditModal = (company) => {
    setEditorState({ open: true, mode: 'edit', initialValues: company });
  };
  ...
  const handleEditorSubmit = async (payload) => {
    ...
    try {
      if (editorState.mode === 'create') {
        ...
        setStatusMessage('Company workspace created successfully.');
      } else if (editorState.initialValues?.id) {
        await updateAdminCompany(editorState.initialValues.id, payload);
        setStatusMessage('Company workspace updated.');
      }
      setEditorState({ open: false, mode: 'create', initialValues: null });
      await loadCompanies();
    } catch (submitError) {
  ...
  const handleArchive = async () => {
    ...
    setStatusMessage('');
    try {
      await archiveAdminCompany(confirmArchive.id);
      setStatusMessage(`Archived ${confirmArchive.companyName}.`);
      setConfirmArchive(null);
      await loadCompanies();
    } catch (archiveError) {
      setError(archiveError instanceof Error ? archiveError.message : 'Unable to archive company.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return [
      {
        ...
        description: 'Corporate workspaces with admin governance.',
      },
      {
        title: 'Active',
        value: summary.statuses?.active ?? 0,
        description: 'Companies actively collaborating across Gigvora.',
      },
      {
        title: 'With website',
        value: summary.withWebsite ?? 0,
        description: 'Organisations with a published marketing destination.',
      },
      {
        title: 'Contact ready',
        value: summary.withContact ?? 0,
        description: 'Workspaces with verified contact channels.',
      },
    ];
  }, [summary]);

  return (
    <section id="admin-company-management" className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-slate-900">Company management</h2>
          <p className="mt-1 text-sm text-slate-500">
            Accelerate B2B onboarding with high-fidelity company records, verified contacts, and broadcast-ready brand assets.
          </p>
        </div>
      ...
      {error ? (
        <div className="rounded-3xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700">{error}</div>
      ) : null}
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="divide-y divide-slate-200 text-sm">
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-sm text-slate-500">
                  {filters.search ? 'No companies match the current filters.' : 'No companies onboarded yet.'}
                </td>
              </tr>
            ) : (
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500">{company.location || 'Location pending'}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${
                        STATUS_BADGES[company.owner?.status] ?? 'bg-slate-200 text-slate-600'
                      }`}
                    >
                      {company.owner?.status ?? 'unknown'}
                    </span>
                  </td>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/40 px-4">
          <div className="w-full max-w-md rounded-3xl border border-rose-200 bg-white p-6 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Archive company workspace?</h3>
                <p className="text-sm text-slate-500">
                  The company owner will be suspended and the workspace archived until reactivated by an administrator.
                </p>
              </div>
            </div>
