...
- **Finance distribution dashboard:** Payout hero cards reuse `rounded-4xl` shells with gradient overlays (`from-accent/15 via-white to-emerald-5`) and embed inline sparkline bars coloured `text-emerald-600` for positive deltas or `text-rose-600` for regressions. Upcoming payout table inherits Trust Center density but adds status chips (`bg-emerald-100`, `bg-amber-100`, `bg-rose-100`) with icon glyphs from the finance set. Outstanding split list uses stacked cards with `border-l-4` severity accents and `grid grid-cols-[auto_1fr_auto]` layout for avatar, copy, and action buttons. Distribution donut sits inside a `bg-white/90` card with inset shadow `shadow-[0_20px_45px_-28px_rgba(15,23,42,0.35)]` and legend pills mirroring accent colours. Export checklist uses a vertical timeline motif with accent dots and `border-l border-slate-200`; actionable rows shift to `bg-amber-50` when blockers exist. Quick action buttons adopt tertiary styling (`bg-slate-900 text-white` for urgent payout, `border border-accent` for notify finance) with icon-leading alignment for clarity.
