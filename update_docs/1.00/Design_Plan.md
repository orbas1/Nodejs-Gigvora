...
- **Profile Experience**: Editor overlays capture headline, skills, availability, trust breakdown, references, and programme badges. Payload contracts now mirror backend sanitisation—skills serialise to JSON strings, trust-score breakdown metadata is surfaced inline, and availability banners share the same schema used by Experience Launchpad and Volunteer dashboards.
