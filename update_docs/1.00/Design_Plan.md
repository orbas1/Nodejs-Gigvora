...
- Profile trust and targeting telemetry now consumes dedicated events (`profile.trust_score.updated`, `profile.engagement.metrics_refreshed`, `profile.targeting.funnel_updated`), aligning design annotations with the backend measurement framework so dashboards and funnels render consistent insights.
