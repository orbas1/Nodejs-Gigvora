...
- Mirror the maintenance telemetry and refresh guidance on Flutter splash/login flows so mobile users receive the same runtime
  status messaging documented for web/admin surfaces. Updates recorded in `App_screens_drawings.md`, `Screens_Update_Plan.md`,
  and `Screen_text.md`.
...
- Document payments/compliance guard states across finance and compliance dashboards, surfacing downtime banners, request IDs, and escalation copy when runtime dependency checks pause wallet or document workflows.
- Capture custodial dependency outage states for finance, payout, and verification modules so the new backend guards render consistent 503 banners, disabled CTAs, and recovery messaging; annotate behaviour in `Dashboard Designs.md`, `component_functions.md`, and `App_screens_drawings.md` for Flutter parity.
- Map bounded-context ownership in UI artefacts by updating `Architecture_Domain_Map.md` and in-product ERDs so dashboard widgets reference the new auth/marketplace/platform domains.
- Design domain registry observability panels that visualise `/api/domains` responses with drill-down tables, schema change alerts, and export actions documented in `Screens_Updates_widget_functions.md` and `Dashboard Designs.md`.
- Extend admin operations specs with runtime health telemetry sourced from `/api/admin/runtime/health`, including dependency badges, rate-limit utilisation bars, connection pool meters, Prometheus exporter freshness indicators, and manual refresh behaviours recorded in `Dashboard Designs.md`, `component_functions.md`, and `Screen_text.md`.
- Add an operator-facing documentation download CTA to the runtime telemetry shell, referencing `/api/docs/runtime-security` with tooltip/empty-state copy documented in `Dashboard Designs.md`, `component_functions.md`, and `Screen_text.md` to keep handoffs aligned with the published API contract.
- Document shutdown audit overlays that confirm database drain status, referencing `component_functions.md` and `Screen_text.md` for copy and ARIA guidance when maintenance playbooks initiate connection draining, including new failure guidance triggered by the backend shutdown orchestrator when drains error.
...
- Capture exporter-stale escalation messaging and runbook CTA placement so admin users can open `runtime-incident` guidance directly from the telemetry card when Prometheus scrapes fail; specs recorded in `Screen_text.md`, `Screen_buttons.md`, and `Dashboard Designs.md`.
- Add an operator-facing documentation download CTA to the runtime telemetry shell, referencing `/api/docs/runtime-security` with tooltip/empty-state copy documented in `Dashboard Designs.md`, `component_functions.md`, and `Screen_text.md` to keep handoffs aligned with the published API contract.
- Document shutdown audit overlays that confirm database drain status, referencing `component_functions.md` and `Screen_text.md` for copy and ARIA guidance when maintenance playbooks initiate connection draining, including new failure guidance triggered by the backend shutdown orchestrator when drains error.
- Define admin maintenance registry workflows (list, modal editor, lifecycle toasts) referencing `Dashboard Designs.md`, `Screens_Updates_widget_functions.md`, and `Application_Design_Update_Plan/Screens_Updates_widget_functions.md` so CRUD flows mirror the new backend announcement schema.
