...
- **1.B.1. `Header.jsx`**
  ...
  4. **Redundancies.** Inbox preview now pulls threads through shared messaging helpers so the dock and header stay aligned while backend messaging tables expose matching preview fields—keep consolidating formatting helpers so badge states and metadata evolve together.【F:gigvora-frontend-reactjs/src/components/Header.jsx†L37-L158】【F:gigvora-backend-nodejs/database/migrations/20240615080000-data-model-expansion.cjs†L166-L212】【F:gigvora-backend-nodejs/src/models/messagingModels.js†L52-L108】
  5. **Placeholders / Stubs.** `loadInboxPreview` streams real data via `fetchInbox` with abort handling, backed by seeded threads for demo accounts so no mock content remains in the navigation surface.【F:gigvora-frontend-reactjs/src/components/Header.jsx†L452-L489】【F:gigvora-frontend-reactjs/src/services/messaging.js†L9-L41】【F:gigvora-backend-nodejs/database/seeders/20240501010000-demo-data.cjs†L132-L302】【F:gigvora-backend-nodejs/database/seeders/20240501010000-demo-data.cjs†L604-L778】
